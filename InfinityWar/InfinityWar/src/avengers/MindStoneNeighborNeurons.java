...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        String[] neurons = new String[vertices];
        for(int i = 0;i<vertices;i++) neurons[i] = StdIn.readString();
        StdOut.setFile(outputFile);
        int number = StdIn.readInt();
        for(int i = 0 ;i<number;i++){
            String front = StdIn.readString();
            String back = StdIn.readString();
            if(back.equals(neurons[vertices-1])) StdOut.println(front);
        }
    }
}
