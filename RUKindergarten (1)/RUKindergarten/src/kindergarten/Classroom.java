...
public class Classroom {
    ...
    public void enterClassroom(String filename) {
        ...
        for (int i = 0; i < length; i++) {
            SNode studentsLeft = new SNode(new Student(StdIn.readString(), StdIn.readString(), StdIn.readInt()), studentsInLine);
            studentsLeft.setNext(studentsInLine);
            studentsInLine = studentsLeft;
        }
    }
    ...
    public void seatStudents() {
        ...
        int rowLength = seatingLocation.length;
        int colLength = seatingLocation[0].length;
        for (int i = 0; i < rowLength; i++) {
            for (int j = 0; j < colLength; j++) {
                if (seatingLocation[i][j] && studentsInLine != null && studentsSitting[i][j] == null) {
                    node = studentsInLine;
                    studentsInLine = studentsInLine.getNext();
                    studentsSitting[i][j] = node.getStudent();
                }
            }
        }
    }
    ...
    public void insertMusicalChairs() {
        ...
        // WRITE YOUR CODE HERE
        SNode head = null;
        int rowLength = studentsSitting.length;
        int colLength = studentsSitting[0].length;
        for (int i = 0; i < rowLength; i++) {
            for (int j = 0; j < colLength; j++) {
                if (studentsSitting[i][j] != null) {
                    Student newStudent = studentsSitting[i][j];
                    studentsSitting[i][j] = null;
                    if (head != null) {
                        SNode tail = new SNode(newStudent, musicalChairs);
                        head.setNext(tail);
                        head = head.getNext();
                    } else {
                        musicalChairs = new SNode(newStudent, musicalChairs);
    ...
    public void moveStudentFromChairsToLine(int size) {
       int index = StdRandom.uniform(size);
        ...
        Student newStudent = remove.getStudent();
        insertByHeight(newStudent);
    }
    ...
    public void insertByHeight(Student studentToInsert) {
        ...
        SNode previousNode= studentsInLine;
        SNode currentNode = studentsInLine.getNext();
    ...
    public void seatMusicalChairsWinner() {
        ...
        // WRITE YOUR CODE HERE
        if(musicalChairs != null || musicalChairs.getNext().getStudent() == musicalChairs.getStudent())
        {
            Student winnerStudent = musicalChairs.getStudent();
            int rowLength = seatingLocation.length;
            int colLength = seatingLocation[0].length;
            for (int i = 0; i < rowLength; i++) {
                for (int j = 0; j < colLength; j++) {
                  if (seatingLocation[i][j] == true) {
                      studentsSitting[i][j] = winnerStudent;
                      musicalChairs = null;
                      return;
                    }  
