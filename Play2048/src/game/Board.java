...
public class Board {
    ...
    public void addRandomTile() {
        ...
        gameBoard[t.getRow()][t.getCol()] = tile;
    }
    ...
    public void swipeLeft() {
        ...
        for (int row = 0; row < rowLength; row++) {
            int zeroIndexPosition = 0;
            for (int col = 0; col < colLength; col++) {
                if (gameBoard[row][col] != 0) {
                    if (zeroIndexPosition != col) {
                        gameBoard[row][zeroIndexPosition] = gameBoard[row][col];
                        gameBoard[row][col] = 0;
                    }
                    zeroIndexPosition++;
                }
    ...
    public void mergeLeft() {
        ...
        for (int row = 0; row < rowLength; row++) {
                for (int col = 0; col < colLength; col++) {
                if ((gameBoard[row][col] != 0) && ((col + 1) < colLength)) {
                    if ((gameBoard[row][col] == gameBoard[row][col + 1]) && (gameBoard[row][col] != 0)) {
                        gameBoard[row][col] *= 2;
    ...
    public void transpose() {
        ...
        int tmpI;
        for(int col = 0; col < colLength; col++) {
            for (int row = col; row < rowLength; row++) {
                tmpI=0;
                tmpI = gameBoard[row][col];
                gameBoard[row][col] = gameBoard[col][row]; 
                gameBoard[col][row] = tmpI;
            }
        }
    }
    ...
    public void flipRows() {
        ...
        for(int row = 0;row<rowLength;row++){
            int tmpI = 0;
            for(int col1 = 0,col2 = colLength-1; col1<colLength/2; col1++, col2--){
                tmpI = gameBoard[row][col1];
                gameBoard[row][col1] = gameBoard[row][col2];
                gameBoard[row][col2]=tmpI;
            }
        }
